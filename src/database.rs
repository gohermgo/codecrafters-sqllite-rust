...
fn btree_page_read_header_inner<R: io::Read>(
    ...
    }: BTreePageHeaderInner<u8> = unsafe { core::mem::transmute(buf) };
...
pub fn btree_page_read_header<R: io::Read>(r: &mut R) -> io::Result<BTreePageHeader> {
    let inner = btree_page_read_header_inner(r)?;
